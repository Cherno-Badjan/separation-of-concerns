const pool = require('../utils/pool');

// static methods -> Order.insert(): Math.random(), Number.parseInt(), JSON.stringify(). methods that are on the class itself.
// instance methods -> arr.map(), params.get('code'). Methods on an instance. 
module.exports = class Order {
  id;
  quantity;

  constructor(row) {
    this.id = row.id;
    this.quantity = row.quantity;
  }

  static async insert(order) {
    const {
      rows,
    } = await pool.query(
      'INSERT INTO orders (quantity) VALUES ($1) RETURNING *',
      [order.quantity]
    );

    return new Order(rows[0]);
  }
  static async select() {
    const {
      rows,
    } = await pool.query(
      'SELECT * FROM orders',
    );
// Map through array of rows and for each row, create a new instance of the Order class.
    return rows.map(row => new Order(row))
  }
  static async selectById(id) {
    const {
      rows,
    } = await pool.query(
      'SELECT * FROM orders WHERE id=$1', [id],
    );

    return new Order(rows[0])
  }
  static async updateById({id, quantity}) {
    const {
      rows
    } = await pool.query(
      'UPDATE orders SET quantity = $1 WHERE id=$2 RETURNING *', [quantity,id],
    );

    return new Order(rows[0])
  }
  static async deleteById({id, quantity}) {
    const {
      rows
    } = await pool.query(
      'DELETE FROM orders WHERE quantity = $1 WHERE id=$2 RETURNING *', [quantity,id],
    );

    return new Order(rows[0])
  }
};
